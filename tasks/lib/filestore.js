      ...
      if (me._oOptions.ui5.transport_use_locked) {
        if (oResponse.body) {
          ...
          if (aMatched && aMatched.length > 0) {
            if (!me._oOptions.ui5.transportno) {
              me._oLogger.log(
                "NW ABAP UI5 Uploader: Warning: the current BSP Application was already locked in " +
                  aMatched[0] +
                  ". Now we are using transport " +
                  aMatched[0] +
                  " instead of " +
                  me._oOptions.ui5.transportno +
                  "."
              );
              ...
              me._oLogger.log(
                "NW ABAP UI5 Uploader: Warning the current BSP application seems to be locked in several transports. Please merge them to avoid inconsistency"
              );
              fnCallback(
                "The current BSP application seems to be locked in several transports. Please merge them to avoid inconsistency",
                null
              );
              return;
            }
